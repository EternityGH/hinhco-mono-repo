                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            ...
                            @if($auth_user->can('serviceaddon list'))
                            <a href="{{ route('serviceaddon.index') }}" class=" float-end btn btn-sm btn-primary"><i class="fa fa-angle-double-left"></i> {{ __('messages.back') }}</a>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                ...
                                <div class="custom-file">
                                    <input type="file" name="serviceaddon_image" class="custom-file-input" onchange="previewImage(event)" accept="image/*" required>
                                    @if($serviceaddon && getMediaFileExit($serviceaddon, 'serviceaddon_image'))
                                    <label class="custom-file-label upload-label">{{ $serviceaddon->getFirstMedia('serviceaddon_image')->file_name }}</label>
                                    @else
                            ...
                            <div class="col-md-2 mb-2">
                                <div class="image-preview-container">
                                    <img id="serviceaddon_image_preview" src="{{ getMediaFileExit($serviceaddon, 'serviceaddon_image') ? getSingleMedia($serviceaddon, 'serviceaddon_image') : '' }}" alt="Image preview" class="attachment-image mt-1" style="width: 150px; {{ getMediaFileExit($serviceaddon, 'serviceaddon_image') ? '' : 'display: none;' }}">
                                    <a class="text-danger remove-file" id="removeButton" onclick="removeImage(event, '{{ route('remove.file', ['id' => $serviceaddon->id, 'type' => 'serviceaddon_image']) }}')" style="{{ getMediaFileExit($serviceaddon, 'serviceaddon_image') ? 'display: inline;' : 'display: none;' }}">
                        ...
                        {{ Form::submit(trans('messages.save'), ['class' => 'btn btn-md btn-primary float-end', 'id' => 'saveButton']) }}
                        {{ Form::close() }}
                    </div>
                </div>
            </div>
