...
@section('content')
    @php
        ...
        $serviceIds = collect($postJobData['post_request_detail']['service'])
            ->pluck('id')
            ->toArray();
    @endphp
                ...
                <div class="col-12">
                        ...
                        <div class="row align-items-center">
                                ...
                                <div class="d-flex align-items-center">
                                    @if ($postJobData['post_request_detail']['status'] == 'assigned')
                                        <h5 class="m-0 text-primary">
                                            {{ getPriceFormat($postJobData['post_request_detail']['job_price']) }}</h5>
                                        <span class="text-primary text-capitalize">(job price)</span>
                                    @else
                                        <h5 class="m-0 text-primary">
                                            {{ getPriceFormat($postJobData['post_request_detail']['price']) }}</h5>
                            ...
                            <div class="col-lg-4">
                                @if (!empty($postJobData['post_request_detail']['service'][0]['attchments']))
                                    <img src="{{ $postJobData['post_request_detail']['service'][0]['attchments'][0] }}"
                                        class="img-fluid w-100 rounded-3" alt="post-job-image">
                                @else
                            ...
                            @foreach ($postJobData['post_request_detail']['service'] as $service)
                                ...
                                <div class="text-center d-inline-block">
                                    @if (!empty($service['attchments']))
                                        <img src="{{ $service['attchments'][0] }}"
                                            class="img-fluid avatar-120 rounded-2 object-fit-cover" alt="booking-service">
                                    @else
                                        <img src="{{ asset('images/default.png') }}" alt=""
                                            class="img-fluid object-fit-cover rounded-3 mt-4 w-100" />
                                    @endif
                                ...
                                @foreach ($postJobData['post_request_detail']['service'] as $service)
                                    <div class="slider-items"> <!-- === Swiper Slider Slide === -->
                                        <div class="text-center d-inline-block">
                                            @if (!empty($service['attchments']))
                                                <img src="{{ $service['attchments'][0] }}"
                                                    class="img-fluid avatar-120 rounded-2 object-fit-cover"
                                                    alt="booking-service">
                                            @else
                                                <img src="{{ asset('images/default.png') }}" alt=""
                                                    class="img-fluid object-fit-cover rounded-3 mt-4 w-100" />
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-between gap-3">
                                                        <div class="d-inline-flex align-items-center gap-3">
                                                            <div class="flex-shrink-0">
                                                                <img src="{{ $bidderData['provider']['profile_image'] }}"
                                                                    class="avatar-70 rounded-circle object-cover"
                                                                    alt="bidder-image">
                                                            </div>
                                                            <div class="content">
                                                                <h6 class="mt-0 mb-1 text-capitalize font-size-18">
                                                                    {{ $bidderData['provider']['display_name'] }}</h6>
                                                                <p class="mt-0 mb-2 text-capitalize font-size-14 lh-1">
                                                                    {{ $bidderData['provider']['designation'] }}</p>
                                                                <div>
                                                                    <rating-component :readonly="true"
                                                                        :showrating="false"
                                                                        :ratingvalue="{{ $bidderData['provider']['providers_service_rating'] }}" />
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="add-button text-end">
                                                            <h5 class="text-capitalize text-priamry">
                                                                {{ getPriceFormat($bidderData['price']) }}</h5>
                                                            <div class="mt-2">
                                                                <span
                                                                    class="badge rounded-3 py-2 px-3 text-bg-primary">Assigned</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </li>
                            ...
                            <div class="row">
                                @foreach ($postJobData['bider_data'] as $bidderData)
                                    <form method="POST" id="updatepostjobForm">
                                        <input type="hidden" id="post_request_id"
                                            value="{{ $bidderData['post_request_id'] }}">
                                        <input type="hidden" id="provider_id" class="provider_id"
                                            value="{{ $bidderData['provider_id'] }}"
                                            data-provider-id="{{ $bidderData['provider_id'] }}">
                                        <input type="hidden" id="price" class="price"
                                            value="{{ $bidderData['price'] }}" data-price="{{ $bidderData['price'] }}">
                                                    ...
                                                    <div class="d-flex align-items-center justify-content-between gap-3">
                                                        ...
                                                        <div class="add-button text-end">
                                                            ...
                                                            @if ($postJobData['post_request_detail']['status'] == 'requested')
                                                                <div class="mt-2">
                                                                    <button type="button"
                                                                        class="btn btn-primary px-5 text-capitalize d-block update_post_job">accept</button>
                                                                </div>
                                                            @endif
                                                        </div>
                                                    </div>
                    ...
                    @if (!empty($postJobData['post_request_detail']['provider_id']))
                        <div class="text-center my-4">
                            <a href="{{ route('book.post_job', ['id' => $postJobData['post_request_detail']['id']]) }}"
                                class="btn btn-lg btn-primary continue-button">Book Now</a>
                        </div>
                    @endif
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                ...
                                window.location.href =
                                    `${baseUrl}/post-job-detail/${postRequestId}`;
                            }
                        })
                    },
