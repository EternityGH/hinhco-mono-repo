            ...
            <div class="row">
                                        ...
                                        <div
                                            ...
                                            <div class="icon text-primary">
                                                <svg width="26" height="27" viewBox="0 0 26 27" fill="none"
                                                    ...
                                                    <mask style="mask-type:luminance" maskUnits="userSpaceOnUse" x="2"
                                                        y="2" width="22" height="23">
                                            ...
                                            <div class="content">
                                                ...
                                                <a
                                                    href="mailto: {{ optional($generalsetting)->inquriy_email }}">{{ optional($generalsetting)->inquriy_email }}</a>
                                            </div>
                                        </div>
                                    @endif
                                    @if (optional($generalsetting)->helpline_number)
                                        ...
                                        <div
                                            ...
                                            <div class="content">
                                                ...
                                                <a
                                                    href="tel:{{ optional($generalsetting)->helpline_number }}">{{ optional($generalsetting)->helpline_number }}</a>
                                            </div>
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="mt-5 pt-lg-5">
                                <ul class="iq-social-list-text d-flex align-items-center flex-wrap m-0 list-inline">
                                    @if (optional($socialmedia)->facebook_url)
                                        <li class="me-3 pe-3">
                                            <a href="{{ optional($socialmedia)->facebook_url }}"
                                                target="_blank">{{ __('landingpage.facebook') }}</a>
                                        </li>
                                    @endif

                                    @if (optional($socialmedia)->twitter_url)
                                        <li class="me-3 pe-3">
                                            <a href="{{ optional($socialmedia)->twitter_url }}"
                                                target="_blank">{{ __('landingpage.twitter') }}</a>
                                        </li>
                                    @endif
                                    ...
                                    @if (optional($socialmedia)->instagram_url)
                                        <li class="me-3 pe-3">
                                            <a href="{{ optional($socialmedia)->instagram_url }}"
                                                target="_blank">{{ __('landingpage.instagram') }}</a>
                                        </li>
                                    @endif
                                    ...
                                    @if (optional($socialmedia)->youtube_url)
                                        <li class="me-3 pe-3">
                                            <a href="{{ optional($socialmedia)->youtube_url }}"
                                                target="_blank">{{ __('landingpage.youtube') }}</a>
                                        </li>
                                    @endif

                                    @if (optional($socialmedia)->linkedin_url)
                                        <li>
                                            <a href="{{ optional($socialmedia)->linkedin_url }}"
                                                target="_blank">{{ __('landingpage.linked_in') }}</a>
                                        </li>
                                    @endif
                                </ul>
                            </div>
                ...
                @if ($sectionData && isset($sectionData['footer_setting']) && $sectionData['footer_setting'] == 1)
                    <div class="col-lg-7 custom-border-left mt-lg-0 mt-5 pt-lg-0 pt-3">
                        @if (
                            $sectionData['footer_setting'] == 1 &&
                                isset($sectionData['enable_popular_category']) &&
                                $sectionData['enable_popular_category'] == 1)
                            <div class="footer-inner-box position-relative ps-lg-5 h-100">
                                <h5 class="text-white">{{ __('landingpage.handyman_category') }}</h5>
                                <div class="mt-3">
                                    <ul
                                        class="iq-footer-catogery-list d-flex align-items-center flex-wrap m-0 list-inline">
                        ...
                        @php
                            ...
                            $sectionData = $footerServiceSection
                                ? json_decode($footerServiceSection->value, true)
                                : null;
                        @endphp
                        @if (
                            $sectionData &&
                                isset($sectionData['footer_setting']) &&
                                $sectionData['footer_setting'] == 1 &&
                                isset($sectionData['enable_popular_service']) &&
                                $sectionData['enable_popular_service'] == 1)
                            <div class="mt-5">
                                ...
                                <ul class="list-inline mt-3 mb-0 d-flex flex-wrap gap-5 popular-service-list">
                                    @foreach ($sectionData['service_id'] as $serviceId)
                                        @php
                                            ...
                                            $mediaServiceImages = $service
                                                ? $service->getMedia('service_attachment')
                                                : null;
                                        @endphp
                                            ...
                                            <li>
                                                <div class="text-center">
                                                    <a href="{{ route('service.detail', $service->id) }}"
                                                        class="text-body">
                                                        <img src="{{ url($mediaServiceImages->first()->getUrl()) }}"
                                                            alt="service-image" style="width: 100px; height: auto;">
                                                        <span
                                                            class="mt-2 line-count-2 popular-service-text">{{ $service->name }}</span>
                                                    </a>
                                                </div>
                                            </li>
                                        @endif
            ...
            <div class="row align-items-center">
                ...
                <div class="col-md-6 text-md-end text-center">
                    <span class="d-inline-flex align-items-center gap-3 flex-wrap">
                        <a target="_blank" href="{{ route('user.term_conditions') }}"
                            class="text-body link-primary">{{ __('landingpage.terms_conditions') }}</a>
                        <a target="_blank" href="{{ route('user.privacy_policy') }}"
                            class="text-body link-primary">{{ __('landingpage.privacy_policy') }}</a>
                        <a target="_blank" href="{{ route('user.help_support') }}"
                            class="text-body link-primary">{{ __('landingpage.help_support') }}</a>
                        <a target="_blank" href="{{ route('user.refund_policy') }}"
                            class="text-body link-primary">{{ __('landingpage.refund_policy') }}</a>
                        <a target="_blank" href="{{ route('user.data_deletion_request') }}"
                            class="text-body link-primary">{{ __('landingpage.data_deletion_request') }}</a>
                    </span>
                </div>
            </div>
        </div>
