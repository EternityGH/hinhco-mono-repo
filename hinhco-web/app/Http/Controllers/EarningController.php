        ...
        if ($request->is('api/*')) {
            ...
            foreach ($providers as $provider) {
                ...
                foreach ($commissions as $commission) {
                    ...
                    $commission_data = CommissionEarning::where('booking_id', $commission->booking_id)->where('user_type', 'admin')->where('commission_status', 'unpaid')->first();
                    $adminEarning += $commission_data->commission_amount;
                }

                $totalEarning = $totalEarning ? $totalEarning : '-';

                $adminEarning = $adminEarning ? $adminEarning : '-';
                ...
                $earningData[] = [
                    ...
                    'provider_earning_formate' => $providerEarning,
                ];
            }
        ...
        if ($request->ajax()) {
            return Datatables::of($query)
                ...
                ->editColumn('total_bookings', function ($row) {
                    ...
                    $row['total_bookings'] = $totalBookings;
                    if ($row->total_bookings > 0) {
                        return "<b><a href='" . route('booking.index', ['handyman_id' => $row->id]) . "' data-assign-module='" . $row->id . "'  class='text-primary text-nowrap px-1' data-bs-toggle='tooltip' title='View Photographer Bookings'>" . $row->total_bookings . "</a> </b>";
                    } else {

                        return "<b><span  data-assign-module='" . $row->id . "'  class='text-primary text-nowrap px-1' data-bs-toggle='tooltip' title='View Photographer Bookings'>0</span>";
                    }
